...
def mock_broker(monkeypatch, engine):
    # TODO: temp patch SessionLocal() to use test db, done better in main
...
class TestNotificationWorker:
    ...
    async def test_web_push_delivery_failed(self, db_session, test_user, test_notification, test_subscription,
                                            monkeypatch, test_delivery, mock_broker):
        webpush = AsyncMock(
            return_value=None,
