...
from backend.chat.models import MessageRole, WSMessage
...
class TestRestMessageSending:
    ...
    def test_send_single_message_success(self, test_users, test_channel_id):
        ...
        assert msg.channel_id == test_channel_id
        assert msg.sender_id == user.id
        assert msg.text == text
        assert msg.role == MessageRole.USER
        assert msg.sent_at is not None
    
    def test_send_multiple_messages_same_channel(self, test_users, test_channel_id):
        """Send multiple messages to same channel."""
        user_a, user_b = test_users[0], test_users[1]
        
        msg1 = send_message(test_channel_id, user_a.id, "Message 1")
        msg2 = send_message(test_channel_id, user_b.id, "Message 2")
        msg3 = send_message(test_channel_id, user_a.id, "Message 3")
...
class TestRestMessageRetrieval:
    ...
    def test_get_empty_channel_history(self, test_channel_id):
        """Get messages from empty channel returns empty list."""
        messages = get_messages(test_channel_id)
        
        assert messages == []
    
    def test_get_single_message_history(self, test_users, test_channel_id):
        """Send one message and retrieve it."""
        user = test_users[0]
        text = "Test message"
        
        sent_msg = send_message(test_channel_id, user.id, text)
        retrieved = get_messages(test_channel_id)
        
        assert len(retrieved) == 1
        assert retrieved[0].id == sent_msg.id
        assert retrieved[0].text == text
    
    def test_get_messages_pagination_basic(self, test_users, test_channel_id):
        ...
        # Get first 5
        page1 = get_messages(test_channel_id, limit=5, offset=0)
        assert len(page1) == 5
        assert page1[0].id == msg_ids[0]
        assert page1[4].id == msg_ids[4]
        
        # Get next 5
        page2 = get_messages(test_channel_id, limit=5, offset=5)
        assert len(page2) == 5
        assert page2[0].id == msg_ids[5]
        assert page2[4].id == msg_ids[9]
    
    def test_get_messages_pagination_partial_page(self, test_users, test_channel_id):
        ...
        page1 = get_messages(test_channel_id, limit=5, offset=0)
        page2 = get_messages(test_channel_id, limit=5, offset=5)
        page3 = get_messages(test_channel_id, limit=5, offset=10)
        
        assert len(page1) == 5
        assert len(page2) == 5
        assert len(page3) == 3
    
    def test_get_messages_pagination_out_of_bounds(self, test_users, test_channel_id):
        ...
        page_out_of_bounds = get_messages(test_channel_id, limit=5, offset=100)
        
        assert page_out_of_bounds == []
    
    def test_get_messages_max_limit_enforced(self, test_users, test_channel_id):
        ...
        all_msgs = get_messages(test_channel_id, limit=300, offset=0)
...
class TestRestMessageOrdering:
    ...
    def test_messages_ordered_by_sent_at_ascending(self, test_users, test_channel_id):
        """Messages returned in ascending chronological order (oldest first)."""
        user = test_users[0]
        
        msg1 = send_message(test_channel_id, user.id, "First")
        msg2 = send_message(test_channel_id, user.id, "Second")
        msg3 = send_message(test_channel_id, user.id, "Third")
    ...
    def test_messages_rapid_succession_ordering(self, test_users, test_channel_id):
        ...
        retrieved = get_messages(test_channel_id, limit=50, offset=0)
...
class TestRestSingleMessageLookup:
    ...
    def test_get_message_by_id_success(self, test_users, test_channel_id):
        """Retrieve a specific message by ID."""
        user = test_users[0]
        text = "Unique message for lookup"
        
        sent_msg = send_message(test_channel_id, user.id, text)
        retrieved = get_message_by_id(test_channel_id, sent_msg.id)
    ...
    def test_get_message_by_id_not_found(self, test_channel_id):
        """Lookup non-existent message returns None."""
        fake_msg_id = uuid4()
        
        retrieved = get_message_by_id(test_channel_id, fake_msg_id)
...
class TestRestChannelIsolation:
    ...
    def test_sender_id_not_spoofable_via_service(self, test_users, test_channel_id):
        """Service layer always uses provided user_id, not from payload."""
        user_a, user_b = test_users[0], test_users[1]
        
        # Send as user_a
        msg = send_message(test_channel_id, user_a.id, "Message")
...
class TestRestMessageMetadata:
    ...
    def test_message_has_unique_id(self, test_users, test_channel_id):
        """Each message gets unique UUID."""
        user = test_users[0]
        
        msg1 = send_message(test_channel_id, user.id, "Msg1")
        msg2 = send_message(test_channel_id, user.id, "Msg2")
        
        assert msg1.id != msg2.id
        assert isinstance(msg1.id, uuid4().__class__)
    
    def test_message_has_sent_at_timestamp(self, test_users, test_channel_id):
    ...
    def test_message_role_defaults_to_user(self, test_users, test_channel_id):
