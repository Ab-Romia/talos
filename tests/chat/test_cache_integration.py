...
class TestCacheHotStorage:
    ...
    def test_message_put_appears_in_hot(self, fresh_cache, test_users, test_channel_id):
        """Message put() immediately appears in hot cache."""
        user = test_users[0]
        msg = fresh_cache.__class__.WSMessage(
        ...
        fresh_cache.put(msg)
        hot_msgs = fresh_cache.get_hot(test_channel_id)
        
        assert len(hot_msgs) == 1
        assert hot_msgs[0].id == msg.id
    
    def test_multiple_messages_hot_cache(self, fresh_cache, test_users, test_channel_id):
    ...
    def test_hot_cache_size_limit_eviction(self, fresh_cache, test_users, test_channel_id):
...
class TestCacheColdStorage:
    ...
    def test_message_put_persists_to_cold(self, fresh_cache, test_users, test_channel_id):
        ...
        fresh_cache.put(msg)
        cold_msgs = fresh_cache._load_cold(test_channel_id)
        
        assert len(cold_msgs) == 1
        assert cold_msgs[0].id == msg.id
    
    def test_evicted_messages_in_cold(self, fresh_cache, test_users, test_channel_id):
...
class TestCacheHotColdMerge:
    """Hot + cold cache merging and deduplication."""
    
    def test_get_all_merges_hot_and_cold(self, fresh_cache, test_users, test_channel_id):
        ...
        all_msgs = fresh_cache.get_all(test_channel_id)
        
        assert len(all_msgs) == 110
    
    def test_get_all_deduplicates_messages(self, fresh_cache, test_users, test_channel_id):
    ...
    def test_get_all_sorted_by_sent_at(self, fresh_cache, test_users, test_channel_id):
    ...
    def test_get_all_pagination_across_boundary(self, fresh_cache, test_users, test_channel_id):
        ...
        page = fresh_cache.get_all(test_channel_id, limit=30, offset=40)
...
class TestCacheTTLExpiration:
    ...
    def test_ttl_stale_eviction(self, test_users, test_channel_id):
        """Messages older than TTL removed from hot cache."""
        # Create cache with short TTL
        cache = HotColdCache(ttl=1)  # 1 second TTL
        user = test_users[0]
        
        msg = cache.__class__.WSMessage(
            channel_id=test_channel_id,
            sender_id=user.id,
            text="Old message",
        )
        ...
        cache.put(msg)
        hot_before = cache.get_hot(test_channel_id)
        assert len(hot_before) == 1
        
        # Wait for TTL to expire
        time.sleep(1.1)
        
        hot_after = cache.get_hot(test_channel_id)
        assert len(hot_after) == 0
    
    def test_ttl_expired_messages_still_in_cold(self, test_users, test_channel_id):
        ...
        hot_msgs = cache.get_hot(test_channel_id)
        assert len(hot_msgs) == 0
        
        cold_msgs = cache._load_cold(test_channel_id)
        assert len(cold_msgs) == 1
        assert cold_msgs[0].id == msg.id
    
    def test_get_all_includes_expired_messages(self, test_users, test_channel_id):
        ...
        all_msgs = cache.get_all(test_channel_id)
...
class TestCacheWithServiceLayer:
    ...
    def test_service_send_uses_cache(self, test_users, test_channel_id):
        """Service layer send_message uses cache automatically."""
        user = test_users[0]
        
        msg = send_message(test_channel_id, user.id, "Test via service")
    ...
    def test_service_retrieval_honors_cache_state(self, test_users, test_channel_id):
        ...
        # Get all via service
        all_msgs = get_messages(test_channel_id)
    ...
    def test_service_pagination_across_cache_layers(self, test_users, test_channel_id):
...
class TestCacheGetById:
    ...
    def test_get_by_id_checks_hot_first(self, fresh_cache, test_users, test_channel_id):
        ...
        fresh_cache.put(msg)
        retrieved = fresh_cache.get_by_id(test_channel_id, msg.id)
        
        assert retrieved is not None
        assert retrieved.id == msg.id
    
    def test_get_by_id_falls_back_to_cold(self, fresh_cache, test_users, test_channel_id):
        ...
        retrieved = fresh_cache.get_by_id(test_channel_id, first_msg_id)
