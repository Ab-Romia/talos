...
def middleware_cookie_header():
    """
    Middleware to set Authorization header from cookie.
    For OAuth2PasswordBearer to work with cookie-based auth.
    """

    async def cookie_to_header(request: fastapi.Request, call_next):
    ...
    return cookie_to_header


def middleware_set_cookie():
    """
    Middleware to set access token in cookie for responses that return an access token.
    """

    async def set_cookie(request: fastapi.Request, call_next):
        response = await call_next(request)

        if response.status_code != status.HTTP_200_OK or "access_token" not in response.body.decode("utf-8"):
            return response

        token = response.body.decode("utf-8").split('"access_token":"')[1].split('"')[0]
...
def authenticate_user(username: str, password: str, db: db_dependency) -> bool | User:
    ...
    if not user or user.email_verified is False or user.deleted_at is not None:
        return False
...
async def google_callback(request: fastapi.Request, db: db_dependency):
    """
    Sign-in with Google
    ...
    if identity:
    ...
    else:  # first time user signs up with Google

        # TODO:
        #  email should become verified
        #  change email???
        #  signup: email & password, signin with google
        user = db.query(User).filter(User.primary_email == email).first()

        if user:
            # TODO: add first time warning (link email)
            pass
        else:  # signup
        ...
        identity = IdentityProvider(
            user_id=user.id, auth_url=AuthUrl.google, sub=google_sub
        )
        db.add(identity)
    ...
    # TODO: return weather signup process should continue
    return {"access_token": access_token, "token_type": "bearer"}


@auth.get("/github/callback")
async def github_callback(request: fastapi.Request, db: db_dependency):
    ...
    if not email:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="GitHub email not available"
        )
    ...
    if identity:
        ...
        identity = IdentityProvider(
            user_id=user.id, auth_url=AuthUrl.github, sub=github_id
        )
        db.add(identity)
...
async def login_for_sudo_token(
...
def password_reset(
        user: user_dependency,
        sudo: Annotated[dict, Depends(login_for_sudo_token)],
        db: db_dependency,
    ...
    current_userpassword = (
        db.query(UserPassword).filter(UserPassword.user_id == user.get("id")).first()
    )

    if not verify_password(current_password, current_userpassword.hashed_password):
        raise HTTPException(
            status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="password not correct"
        )
...
async def CreateUser(db: db_dependency, create_user: CreateUserRequest):
...
async def refresh_access_token(
    ...
    token = create_access_token(
        current_user["username"], current_user["id"], timedelta(minutes=20)
    )
    return {"access_token": token, "token_type": "bearer"}
