...
async def ask_question(channel_id: UUID, body: AskRequest, session: SessionDep):
    ...
    # Resolve the caller's file access up front: retrieval must only see files
    # this user may read (channel-restricted files stay invisible), and any
    # explicitly requested file_ids are intersected with that allow-list.
    ...
    if body.file_ids:
        allowed = allowed & set(body.file_ids)
    file_ids = [str(fid) for fid in allowed]
